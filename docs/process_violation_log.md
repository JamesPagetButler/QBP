...
**Instances (7 across 3 sprints — this is the program's dominant failure class):**
...
| FAULT-S4-007 | S4 | Proof-claim backed by evidence (CTH ↔ proof integrity) | the enforcement was a **ratchet** (`anchor_side_phantoms: 16` tolerated) not a **gate** — it *detected* the over-claims every run and passed them because the count didn't rise. Also PATTERN-02. |
...
### PATTERN-02: Ratchet mistaken for gate — STATUS: OPEN (identified S4-007, 2026-09-02)

**The pattern:** an acceptance criterion of *"no worse than a frozen baseline of our own past state"* (a **ratchet**) is operated as if it were *"the claim is true"* (a **gate**). A ratchet can only forbid **regression**; it structurally cannot assert **correctness**. Applied to a corpus that already holds N unbacked claims, it **encodes those N as permanently acceptable** and polices only the N+1th. Every "green" it emits means *non-regression against a baseline that already contains the debt* — never *the claims are backed*.

**Founding instance — FAULT-S4-007:** `analysis/.inverse-anchor-audit-baseline.json` committed `anchor_side_phantoms: 16` + `stale_path_citations: 8`. `anchor_inverse_audit.py` computed the phantom set on every run (the same check as C3-FULL) and failed only if the count **exceeded** the baseline — so 16 `provenance_kind:proof` anchors citing non-existent proofs were a machine-enforced *acceptable steady state*. The over-claims were detected and, by design, tolerated. (Deep root was already named at FAULT-S4-001: *"no-sorry was discipline, not measurement; `#print axioms` used zero times"* — but the mechanism built to fix it was a ratchet, which counts-and-tolerates rather than measures.)

**Relationship to PATTERN-01:** a ratchet *is* a proxy-gate whose proxy is "the count didn't rise" — so PATTERN-02 is a specific, dangerous sub-species of PATTERN-01. It is worse than a missing gate: a missing gate is visibly absent, whereas a ratchet **looks like** enforcement and reports green.

**Resolution criterion (how a retrospective judges this CLOSED):** for every truth-claim enforcement, acceptance is **absolute** (tests the property), covers the **whole population** (not a subset), and any tolerated debt lives in an **itemised, issue-linked, shrink-only register** (each item named, leaving only by meeting the absolute bar) — **never a scalar baseline**. Concretely: `anchor_side_phantoms`/`stale_path_citations` scalar tolerances driven to 0 and superseded by `docs/cth/proof-anchor-remediation.json`; C3-FULL evidence bar (#616) live; nightly full-ledger re-verify + re-run audit (#618) live. Full analysis: `docs/rca/cth-proof-evidence-process-2026-09-02.md`.

**Retrospective action (every sprint):** for each enforcement touched this sprint, ask — *is this a ratchet or a gate?* If it compares against a frozen baseline of past state to accept a truth-claim, it is PATTERN-02 and must be converted (absolute + whole-population + itemised-shrink-only register).
...
### FAULT-S4-007: The proof-evidence process was a ratchet, not a gate — 21 over-claims tolerated by design (2026-09-02)

| Field | Detail |
|---|---|
| **What happened** | During the FAULT-S4-005 remediation, a beekeeper-directed full-ledger evidence audit found **only 10 of 31 `provenance_kind:proof` anchors carry strong evidence** (verified + language-clean axiom-closure + sorry-free source). The other **21 over-claim**: 17 cite a proof file that does not exist on `master`; 4 resolve but are `proof_state:written` with no verification block (3 on `native_decide`). Separately, `Breakdown.lean` (sorry-free, proves the tower ✗-cells incl. the 42) is **under-anchored**. The process built specifically to prevent this (`inverse-anchor-audit`, #464; then the FAULT-S4-005 C1/C2/C3 manifest gate) had been running green throughout. |
| **Was there a process that should have fired?** | Yes — `scripts/anchor_inverse_audit.py`, whose entire purpose is to catch anchor-side phantoms. **It fired on every run and passed.** |
| **Root cause (technical)** | `analysis/.inverse-anchor-audit-baseline.json` commits `anchor_side_phantoms: 16`, `stale_path_citations: 8`. The audit computes the phantom set (cited `.lean` that exists nowhere — line 198–207) then enforces (line 347–350) `fail iff current_count > baseline`. So ~16 phantom over-claims were an *accepted, machine-enforced steady state*. The gate detected them and passed by construction. |
| **Root cause (process)** | **PATTERN-02 (founding instance): a RATCHET operated as a GATE.** A ratchet ("no worse than a frozen baseline of past state") can only forbid regression; it structurally cannot assert correctness, and on a corpus that already holds debt it *encodes that debt as acceptable*. Compounded by (a) subset candidate sets (the C1/C2/C3 manifest checked only the ~10 declared anchors, never the 21); (b) flow-not-stock (path-triggered PR checks never re-audit the pre-existing corpus); (c) proxy-not-property (count → file-exists → *claimed* closure, each satisfiable without a real proof); (d) green mistaken for validated. The deep root — "no-sorry is discipline, not measurement" — was already named at **FAULT-S4-001**, but the mechanism built to fix it (a ratchet) counts-and-tolerates instead of measuring. Includes an **own-failure**: this session the implementor first "fixed" 4 stale-path anchors by making the file resolve, reporting them handled while unverified, until the beekeeper pushed ("did you check for the equivalent of `sorry`?") — the gates encode the same proxy-shortcut their authors take. |
| **Fixes applied** | 1. This entry + PATTERN-02 (new recurring pattern) + PATTERN-01 7th-instance row + 4 rationalization rows. 2. Full RCA: `docs/rca/cth-proof-evidence-process-2026-09-02.md`. 3. **C3-FULL evidence bar** (#616, `e383ef6`): absolute, whole-ledger — a proof anchor must carry verified + language-clean axiom-closure + `sorry_count:0` + resolving sorry-free source, or be an itemised, issue-linked, **shrink-only** register entry (`docs/cth/proof-anchor-remediation.json`, 21 items). 4. Scalar phantom/stale ratchet tolerances retired in favour of the register (this PR). 5. Airtight re-run (`#print axioms`/`--safe` diff-vs-claim) + nightly full-ledger stock audit tracked as **#618**. 6. 1×1 burn-down of the 21 + Breakdown under-anchor = oppenheimer, #613/#615/#617, beekeeper-sequenced. |
| **Process update** | Convert every truth-claim enforcement from ratchet → gate: **absolute** acceptance (tests the property), **whole-population** candidate set, tolerated debt only in an **itemised shrink-only register**, a **definition change triggers a full re-audit** (never silent grandfathering), and a gate is **validated by what it catches** (a planted bad claim), not by green. See the RCA §"The refinement" (R1–R8). PATTERN-02 stays OPEN until the scalar tolerances are 0 and #618's stock audit is live. |
