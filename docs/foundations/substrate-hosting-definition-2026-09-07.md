...
| (e) Crystal-covariance | (a)–(c) hold at every s ∈ 𝓤; S₃-equivalent crystals are identical physics | ℍ_s is defined for every crystal (`vacuum_hosts_quaternion`); equivariance under every ℓ-fixing automorphism — the G₂ side — is **Lean** (`CrystalHosting.aut_hosting_equivariant`, `aut_image_quatSpan` onto; `CDAut` with map_one/map_re/map_N derived, populated by `cdLift`); the ℤ/2 of the S₃ side (ℓ ↦ −ℓ, `gradeAut`) is **Lean** (`CrystalHosting.gradeAut_hosting_equivariant`, `Hosting.hosting_equivariant_grade`); the order-3 elements of S₃ **fix ℓ** (`aut_s3.py`: rot(±120°, det +1) — an earlier version of this row wrongly said they move ℓ) and are now **Lean** too: `CrystalHosting.rotAut3` (ρ³ = id, ρ ℓ = ℓ, multiplicative — the nine coefficient identities close only at 2π/3), `rotAut3_hosting_equivariant`, `Hosting.hosting_equivariant_rot3`; the residue owned by #639 (e) is Brown's Aut(𝕊) = G₂ × S₃ and that `gradeAut` and ρ generate the S₃ — not claimed; the S³ H-space transports to every unit sphere of ℍ_s (Agda wiring, to do) |
...
- It does not identify ℍ_s with "the observer's ℍ" (DERIV-holographic, flag 3). The theorem parts are proved on the sibling PR #642 (`HolographicSubalgebra.lean`, in review — not on master until it merges: L_xL_y = L_{xy} exactly on associative sets; quaternion frames are associative subalgebras; no associative subalgebra of 𝕆 properly contains a quaternion frame; codimension 4 — frame-relative, the honest ceiling until an inner product is registered as its own foundational PR); the postulate "observers require associativity" stays a postulate; "holographic boundary" is interpretation, to be defined and discussed next.
...
| Clause | Kill |
|---|---|
| (a) | the Agda transport of the S³ witnesses fails at some crystal: for some s ∈ 𝓤 no H-space structure on the unit sphere of ℍ_s is reachable from `S3FromCD.S³-HSpace` by the isomorphism ℍ_s ≅ ℍ, or the transported hedgehog has degree ≠ 1 |
| (b) | once the hosting category is defined (open question 1): the H-space product fails to be a morphism of U(s) for some s. Until then this kill cannot fire — recorded as such, not as a pass |
