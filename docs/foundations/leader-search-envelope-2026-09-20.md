...
**Status:** analysis note written by qbp-oppenheimer at the beekeeper's direction ("do a little back of the envelope on how big the search space is, what would constrain it, and what experimental data constrains it — make sure you've really searched the CTH"). Every number below is an order-of-magnitude estimate under a stated assumption. **Nothing here is ruled, anchored or encoded.** Inputs: the proved rule-flow theorems (PR #667), tests 1–2 (#635), and an exhaustive scripted pass over the ledger (master 6.1.0: 300 anchors — the first pass ran against a stale 5.4.0 checkout with 226; the 74 anchors added since contain exactly one provenance-E record, a topology reference, so the data table below is complete against master; every MEAS-/OBS-/PRED-/CONSTRAINT-/REF- record read; keyword search for hydrogen, electron, speed of light, Rydberg, Lamb, g-factor, ℏ, drift).
...
**Finding F4 (structural, positive — under the integer-power assumption of §2).** The single Hessian rate, together with the integer-power map assumption, turns PRED-correlated-alpha-G from "some functional correlation" into a sharp form: ΔG/G ∝ (Δα/α)^{p_G/p_α} with a ratio of small integers, and every drift ratio R_XY = p_X/p_Y. The rate alone supplies only the shared time scale; the integers come from the assumed map. CONJ-vconstants-keystone's R_αμ is then constrained to be rational with small numerator and denominator. Testable only on a detection (the 0/0 caveat stands), but it is a QBP fingerprint the ledger did not yet have.
