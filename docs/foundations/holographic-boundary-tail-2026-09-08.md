...
**Why "dynamics need associativity" was retracted (round 14/15).** The round-13 claim was that autonomous dynamics require associative-ish composition, so physics could only live on ℍ. The test refuted it: the generic non-associative subalgebra *does* carry the algebra's own self-maps. What the maps do is governed by **norm multiplicativity, not associativity**: `R_ℓ` — like `R_t` for *every* `t ∈ 𝕆 ∪ 𝕆ℓ`, octonions included, all non-associative — is an isometry (`generic_maps_check.py` §3: "this holds for every t in 𝕆 ∪ 𝕆ℓ (norm multiplicative there) — what singles out ℓ is [x,x,ℓ] = 0"), so its normalised iteration is a rotation, a symmetry, no motion. A `t` that mixes the two Cayley–Dickson halves gives a non-isometric skew map (`N(xt) ≠ N(x)N(t)`, Prop 7′), whose normalised iteration transients onto its top-singular plane — on the zero-divisor ridge `V = 1` — and is then period 2 up to sign (run of 2026-09-08, 5 generic `t` × 40 starts × 120 iterations; uncommitted scratch log, the committed script's §2 is the 1-`t` version). So both regimes end in symmetries; they differ in whether there is a transient, and the operative axis is Prop 7′, not associativity. Both round-13 claims were retracted (transcript round 15 §1). "Observers require associativity" therefore cannot be read as a dynamical necessity — associativity is not even the relevant variable for the maps.
...
| **T2** maximality | an associative subalgebra **of 𝕆** containing a quaternion frame equals it (dim 4) | proved in `CDAlg ℝ 3`, frame-relative (`span4_eq_of_associative`); global ≤ 4 in 𝕆 deferred; **the same question in 𝕊** (does any associative subalgebra of 𝕊 properly contain `ℍ_s`, which straddles the CD halves?) is open — §6 (ix) |
| **T3** the gap | `dim 𝕆 − dim ℍ = 4` | proved (`quaternion_frame_codim_four`) |
| **P** the postulate | an *observer* `O` in `U(s)` is a subset `A_O ⊆ 𝕊` on which `O`'s actions compose; by T1 `A_O` is associative; by T2 it lies inside one ℍ *if `A_O ⊆` an octonion* (in 𝕊 at large: open, §6 (ix)) | not provable — the entire content of "observers require associativity" |
...
**The hole in P.** "Observer = associative subset" is a *definition*, not a claim, unless *observer* is defined independently first (with T1 in hand, P otherwise collapses to "associative subsets are associative"). The ledger never did that. **Hosting does.** Clause (a) of the hosting definition (`substrate-hosting-definition-2026-09-07.md` §2; the frame the beekeeper ruled 2026-09-07) says an *entity* of `U(s)` is a persistent configuration on the unit sphere `S³ ⊂ ℍ_s` with a topological charge (Agda `SkyrmionCharge`: `B(hedgehog) = 1`, `π₃(S³) ≅ ℤ`). Observers are made of entities. Hence:

> **Definition D.** *The observer's algebra* is the hosted algebra of the universe whose unit sphere carries the observer's configurations (clause (a)) and whose H-space product composes them (clause (b)). This is a definition; without it "the observer's ℍ" has no formal referent.
>
> **P1, reduced (drafted on the beekeeper's direction of 2026-09-09 — "Go ahead with a for P1" — pending the flag-3 ruling, §7 Q3).** Under D and hosting clause (a), the observer's algebra is `ℍ_s` **by construction**, and the justification "observers require associativity" is replaced by "matter is hosted on the crystal's ℍ". What remains as an assumption is
> **P1′:** *observers are entities in the sense of clause (a)* — an observer is a (composite of) persistent charged configuration(s) on `S³ ⊂ ℍ_s` — together with D.

P1′ + D is weaker than P1: it does not identify `ℍ_s` with anything external; it says observers are made of the matter the universe hosts. Hosting already supplies *composition* (clause (b): the H-space product on `S³ ⊂ ℍ_s`, associative by `universe_hosts_quaternion`), so that is **not** P's residual job. What hosting does *not* deliver, and the ledger sentence claims, is the **necessity direction: there are no observers outside the hosted algebra** — no observer lives on a non-associative subset such as the generic 10-dim subalgebra. That converse is the honest residual content of P, and it is exactly what the retracted dynamics argument was reaching for. Honest status: P = the exclusivity claim, a postulate with no dynamical support; P1′ + D = the hosting frame applied to observers.

**Pending, not settled.** This reduction is a *draft under direction*; flag 3 is ruled only when the beekeeper rules §7 Q3. Until then the repo's own guardrails stand and disagree with this section on purpose: `Hosting.lean` (module docstring "What this file does NOT derive" item 2, the `universe_hosts_quaternion` docstring, §11 boundary bullet) and `CrystalHosting.lean` (interpretation guardrail; `vacuum_hosts_quaternion` docstring) all say *"the crystal's ℍ is NOT identified with the observer's ℍ … pending the beekeeper's ruling on flag 3"*. On ratification those six comment sites are owed an update in the same PR that applies the ledger split.

**Pole corollary (derivable under a stated hypothesis).** A pole universe hosts `ℂ = span{1, ℓ}` (`pole_hosts_complex`, master). Its unit sphere is `S¹`, and `π₃(S¹) = 0` (`S¹` is a 1-type — `isGroupoidS¹`, `Cubical/HITs/S1/Properties.agda` in the pinned library — so `πₙ(S¹) = 0` for `n ≥ 2`). **Hypothesis H-dom:** the *domain* of a configuration is a fixed compactified 3-space `S³` while only the *target* degrades with the hosted algebra (this is how clause (a) reads today: `SkyrmeConfig : S³ → S³`, `baryonNumber = degree`). Under H-dom, maps `S³ → S¹` carry no charge: **pole universes host no charged entities, hence no observers.** Under the *other* reading — DERIV-3plus1's "space = the vector part of the hosted algebra", so a pole's space is `Im ℂ ≅ ℝ`, compactified `S¹` — the charge is `π₁(S¹) = ℤ` and poles host charged *kinks*. Which domain clause (a) means is a definition question for #639, not this corollary's to decide. Filed as §6 (viii) with H-dom stated; it needs its own charge type for target `S¹` (`∥ S³ →∙ S¹ ∥₀ ≅ 1`), not a re-use of `baryonNumber` (typed on target `S³`). The pole objection to P1 is therefore an H-dom-conditional theorem, not an exception. P2 is untouched by all of this.
...
**Two premises first** (P2 is not forced by the objects and must be ruled; P1 is reduced in §1a to P1′ + D, drafted on direction and pending the flag-3 ruling):
...
| pole universes host no charged entities, hence no observers — **under H-dom** (fixed `S³` domain) | corollary, **to file** | `pole_hosts_complex` (master) + `π₃(S¹) = 0` (Agda, §6 (viii)); the DERIV-3plus1 domain reading gives `π₁(S¹) = ℤ` instead |
| observers require associativity | **postulate** (P, §1a) | restated: a definition unless *observer* is independent; residual content = exclusivity (no observers outside the hosted algebra); support narrow (§1); not a dynamical necessity (§1a) |
...
8. **(viii) Pole universes carry no topological charge, under H-dom** — Agda: `S¹` is a 1-type (`isGroupoidS¹`, pinned library) ⇒ `πₙ(S¹) ≅ 0` for `n ≥ 2`; a new charge type for target `S¹` (`baryonNumber` is typed on target `S³` and cannot be re-used), showing `∥ S³ →∙ S¹ ∥₀` is contractible; Lean side on master (`pole_hosts_complex`). Cheap once H-dom is fixed; **blocked on the domain question** (§1a) until #639 fixes what the domain of a configuration is. The `agda-prover` lane.
9. **(ix) Frame-maximality in 𝕊** — does any associative subalgebra of 𝕊 properly contain `ℍ_s`? T2 is proved in `CDAlg ℝ 3` only; `ℍ_s` straddles the CD halves, so T2 does not apply to it directly. Needed before P's "lies inside one ℍ" is a statement about 𝕊. Moderate.
