...
RULING = (
    ...
    "was NOT reversed by any attack. Evidence classes now: attack 3 Lean (41 theorems; scope: for X = ℝ with a "
    ...
    "hypothesis interior{V=0} = ∅, which is not discharged in Lean); attack 5 Lean (16 theorems: V's first-order data constrains at most one of the 14 in-flight tangent "
    "directions — exactly one where the rule field is non-zero, none at crystals or on the frozen ridge); attack 1 NUMERICAL + ARGUMENT, Lean pending (anneal = 1/3 exactly by a Laplace "
...
NEW_FLAG = {
    ...
    "description": (
        ...
        "41 theorems, all audited); (5) every level-set invariant is rule-blind: V's first-order data constrains at most one of the 14 "
        "in-flight tangent directions — exactly one where the rule field is non-zero, none at crystals or on the "
        "frozen ridge (TransitionState.lean, 16 theorems); (6) the "
