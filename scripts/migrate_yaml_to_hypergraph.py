...
def migrate_source(kb: QBPKnowledge, data: Dict[str, Any], dry_run: bool = False) -> str:
    ...
    if 'authors' in metadata:
        attrs['authors'] = [a.get('name', a) if isinstance(a, dict) else a
...
def migrate_concept(kb: QBPKnowledge, data: Dict[str, Any], dry_run: bool = False) -> str:
...
def migrate_question(kb: QBPKnowledge, data: Dict[str, Any], dry_run: bool = False) -> str:
...
def infer_hyperedges_from_question(kb: QBPKnowledge, data: Dict[str, Any],
                                    question_id: str, dry_run: bool = False):
    ...
    if len(source_ids) >= 1:
        ...
        if dry_run:
            print(f"  Would add investigation hyperedge: {question_id} + {len(source_ids)} sources")
        else:
            try:
                kb.add_hyperedge(tuple(members), "investigation", {
...
def infer_hyperedges_from_claim(kb: QBPKnowledge, data: Dict[str, Any],
                                 claim_id: str, dry_run: bool = False):
    ...
    if len(source_ids) >= 2:
        ...
        if dry_run:
            print(f"  Would add evidence_chain: {claim_id} + {len(source_ids)} sources (tier {max_strength})")
        else:
            try:
                kb.add_evidence_chain(claim_id, source_ids, confidence_tier=max_strength,
...
def add_lean_proofs(kb: QBPKnowledge, proofs_dir: Path, dry_run: bool = False):
    ...
    for proof in proofs:
        ...
        if dry_run:
            ...
            try:
                kb.add_proof(proof_id, proof['file'],
...
def main():
    ...
    parser.add_argument('--proofs-dir', type=str, default='proofs',
                        help='Path to proofs directory')
    parser.add_argument('--output', type=str, default='knowledge/qbp.hgdb',
