...
Ledger edit through the confined-write helper: only AXIOM-1 (kill_condition — a LIST, one Impasse Record per open
question, per qbp-architecture §I4 — and decision_state; the two D3 fields, nothing else), DERIV-sedenion (ruling), changelog,
...
AXIOM1_KILL = [
    (
        ...
        "(crystallisation, the descent through the zero-divisor locus) or only over processes in "
        "a crystal. KILL — a proven flow on StateSphere (the rule, #635) whose omega-limit set "
        "meets the zero-divisor locus and whose omega-limit map is non-injective on a set of "
        "states of positive normalised surface measure on StateSphere, OR an anchor with "
        "proof_state verified (PROOF-* or DERIV-*) asserting such a loss from a stated process — "
        "then 'no physical process destroys information' is contradicted by a proven process and "
        "AXIOM-1 must change (option A: rescope to the encoding; package §4a price, 22 citers + 5 "
        "principles), because it has been proven that it must. DISCHARGE — the same flow proven "
        "information-preserving (omega-limit map injective almost everywhere with respect to that "
        "measure). First-order semiflows are injective at finite time, so the criterion is on the "
        "omega-limit, not on finite-time states. TODAY — no proven flow exists; the ledger holds "
        "candidate assertions touching the locus, none proven: Prop 16 layer (iii) "
        "(power-iteration transient onto the dominant subspace; numerical, generic_maps_check.py; "
        "not pressure-tested per the 473-ac1-v0.5 addendum), FLAG-seam-dynamics-open (incoherent "
        "placeholder asserting a seam scattering process), DERIV-crystallisation-asymptotic "
        "(names a process and a step counter Gamma but no map on states), DERIV-arrow (takes "
        "information preservation as its premise and asserts a lossy projection — a map; listed "
        "as the ledger's nearest assertion of loss). The kill cannot fire and no contradiction is "
        "derivable: recorded as OPEN, not as a pass. Trigger issue #647; derived there at trigger "
        "time, never ruled."
    ),
    (
        "QUESTION 2 (scope of the selection clause 'selects division algebras') — OPEN, "
        "independent of question 1 (discharging one does not discharge the other): AXIOM-2's "
        "statement applies the selection at the boundary encoding; it does not state that the "
...
SEDENION_RECORD = (
    ...
    "read as ratification of process only, not as the physical reason — a reading PENDING the "
    "beekeeper's own confirmation (asked on PR #659), not asserted as his. Retiring 'seams where "
