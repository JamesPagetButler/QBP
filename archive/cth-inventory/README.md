...
| `baselines/confluent-trust-inventory-v5_24.json` | Historical baseline — **pending intake** | 169 | v0.2 | **Frozen.** QBP-web continued lineage (received 2026-05-31). 28 anchors not in canonical (condensed-math programme, `REF-*` citations, Dirac/Vaidya proofs); missing the 9 foundations appends. Adjudication scope of #509. Lives under `baselines/` because pre-v0.3 exports cannot pass the top-level v0.3 schema lint (`cth-schema-lint.yml` validates `archive/cth-inventory/*.json`); frozen pre-migration baselines are exempt by placement, same as `migrations/` |
