...
`S³-HSpace` and computing with them (e.g. "B(μ 1 ·) = 1") also diverges
(>8 GB); that theorem is deferred as a performance follow-up — the link is
kept at the type level via `S³-carrier-HSpace : HSpace (S₊∙ 3)`.
