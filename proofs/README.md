...
Phase 4a proof structure is exported to JSON (alongside Phase 2 results) for consumption by the Phase 4c interactive visualization. The export script at `src/viz/interactive/export_data.py` extracts proof metadata — theorem names, statement summaries, and the DAG of proof dependencies — and combines it with Phase 2 simulation data into a single JSON file per experiment. Extraction is semi-automated: the script parses `.lean` files for theorem names and dependency structure; human-curated summaries are added as needed. The C/WASM visualization in `src/viz/interactive/` reads this JSON to render proof annotations overlaid on the physics simulation. See [CONTRIBUTING.md](../CONTRIBUTING.md#phase-4c-interactive-proof-visualization) for the full Phase 4c specification.
