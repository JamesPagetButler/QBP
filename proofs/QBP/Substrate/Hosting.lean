...
* **Crystal-covariance across the `S₃` factor of `Aut(𝕊)`.**  `hosting_equivariant`
  covers only automorphisms with `φ ℓ = ℓ` (the G₂ side).  The `S₃` factor moves
  `ℓ` (`CrystalHosting.gradeAut_ell` : `gradeAut ℓ = −ℓ`), so the statements above
  do not apply to it.  Owner: #639 (e).
