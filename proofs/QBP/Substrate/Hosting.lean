...
/-- **The ℤ/2 of the `S₃` side.**  The grade automorphism `gradeAut` (`ℓ ↦ −ℓ`)
    ...
    case of `hosting_equivariant` once constructed as terms of `CDAut 4` — that
    construction remains open (§11). -/
...
* **Crystal-covariance across the order-3 elements of the `S₃` factor of `Aut(𝕊)`.**
  `hosting_equivariant` covers automorphisms with `φ ℓ = ℓ` and
  `hosting_equivariant_grade` covers the reflection `gradeAut` (`ℓ ↦ −ℓ`).  In the
  `S₃ ≅ D₃` description of `Aut(𝕊)/G₂` (Brown 1967; `analysis/473-dirac-probe/aut_s3.py`),
  an element acts on `Im 𝕊 = Im 𝕆 ⊕ ℝℓ ⊕ (Im 𝕆)ℓ` by a matrix `M ∈ O(2)` on the
  multiplicity space of the `7` together with a scalar `s = det M` on `ℓ`.  The
  order-3 elements are the rotations by `±120°`, and they have **`s = +1`: they FIX
  `ℓ`** (it is the three reflections, `s = −1`, that move it — `gradeAut` is one of
  them).  So `hosting_equivariant` already applies to the order-3 elements *as soon
  as they exist as terms of `CDAut 4`*; the open item is that construction, not the
  equivariance argument.  Once it lands, crystal-covariance under the whole `S₃`
  (hosting AC(e)) follows from `hosting_equivariant` + `hosting_equivariant_grade`.
  Owner: #639 (e).
