...
/-! ## 4b. Hosting is an EQUALITY off the poles (#2), and the generic
       intersection of two hosted algebras is `ℂ = span{1, ℓ}` (#3)

`vacuum_hosts_quaternion` proves only the CONTAINMENT `hosted ⊆ ℍ_u` (the
heterogeneous confirmer's 2026-09-19 verdict, row 1: *"a containment, not the
equality the word '=' asserts"*).  This section proves the reverse containment
under the hypothesis the confirmer identified as necessary — the crystal is not
a **pole**, i.e. its `ℓ`-orthogonal part `p = s − s₈·ℓ` is nonzero, equivalently
`(α, γ) ≠ (0,0)` in the Prop-15 parametrisation.  The hypothesis cannot be
dropped: at a pole `s = b₀·ℓ` the hosted algebra is only `span{1, ℓ} ≅ ℂ`
(`Substrate.Hosting.pole_hosts_complex`), which is 2-dimensional, not 4.

The route is the confirmer's: `p ∈ hosted` (`GenByPair.left/.smul/.add` on
`ell_mem_hosted`), then `p·ℓ ∈ hosted` (`GenByPair.mul`), and the pair `(p, p·ℓ)`
is the image of `(U, Uℓ)` under the invertible real matrix `[[α, −γ], [γ, α]]`
— invertible exactly when `α² + γ² ≠ 0`. -/
...
/-- `loOf 1 = 1`. -/
@[simp] theorem loOf_one : loOf (1 : CDAlg ℝ 3) = (1 : CDAlg ℝ 4) :=
  eq_of_halves (by rw [cdLo_loOf, cdLo_one]) (by rw [cdHi_loOf, cdHi_one])

/-- `hiOf 1 = ℓ`. -/
@[simp] theorem hiOf_one_eq_ell : hiOf (1 : CDAlg ℝ 3) = ell := hiOf_one

theorem conj_zero_cd {n : ℕ} : conj (0 : CDAlg ℝ n) = 0 := by
  rw [CDAut.conj_eq_two_re_sub, zero_coord]
  module

/-- **Right multiplication by `ℓ` on the high half:** `(b·ℓ)·ℓ = −b`.  The
    companion of `loOf_mul_ell` (`a·ℓ = hiOf a`), read off the Cayley–Dickson
    doubling formula. -/
theorem hiOf_mul_ell (b : CDAlg ℝ 3) : hiOf b * ell = loOf (-b) := by
  refine eq_of_halves ?_ ?_
  · rw [cdLo_mul, cdLo_hiOf, cdHi_hiOf, cdLo_ell, cdHi_ell, cdLo_loOf, conj_one,
      alt_zero_mul, cd_one_mul, zero_sub]
  · rw [cdHi_mul, cdLo_hiOf, cdHi_hiOf, cdLo_ell, cdHi_ell, cdHi_loOf, conj_zero_cd]
    simp

/-- The `ℓ`-orthogonal part of a parametrised vacuum, in the frame `{U, Uℓ}`:
    `s − s₈·ℓ = α·U + γ·(Uℓ)`. -/
theorem crystal_perp_eq {s : CDAlg ℝ 4} {α γ b₀ : ℝ} (hu0 : u.coord 0 = 0)
    (hlo : cdLo s = α • u) (hhi : cdHi s = b₀ • (1 : CDAlg ℝ 3) + γ • u) :
    s - (s.coord (hiIdx 0)) • ell = α • loOf u + γ • hiOf u := by
  have hs8 : s.coord (hiIdx 0) = b₀ := by
    have h : (cdHi s).coord 0 = b₀ := by
      rw [hhi, add_coord, smul_coord, smul_coord, one_coord, if_pos rfl, hu0, mul_one,
        mul_zero, add_zero]
    rw [← h, cdHi_coord]
  have hplo : cdLo (s - (s.coord (hiIdx 0)) • ell) = α • u := by
  ...
  have hphi : cdHi (s - (s.coord (hiIdx 0)) • ell) = γ • u := by
    rw [cdHi_sub, cdHi_smul, cdHi_ell, hhi, hs8]
    module
  conv_lhs => rw [split_lo_hi (s - (s.coord (hiIdx 0)) • ell)]
  rw [hplo, hphi, loOf_smul, hiOf_smul]

/-- The ⊆ direction of hosting with the parametrisation supplied explicitly (the
    named-`u` core of `vacuum_hosts_quaternion`). -/
theorem genByPair_subset_quatSpan_of_param {s : CDAlg ℝ 4} (hs : s.coord 0 = 0)
    {α γ b₀ : ℝ} (hu0 : u.coord 0 = 0)
    (hlo : cdLo s = α • u) (hhi : cdHi s = b₀ • (1 : CDAlg ℝ 3) + γ • u) :
    ∀ x, GenByPair s ell x → InQuatSpan (loOf u) x := by
  intro x hx
  have hgen := genByPair_ell_mem_quatSpan s hs x hx
  refine inQuatSpan_of_dir (α := α) (γ := -γ) ?_ hgen
  have hP := crystal_perp_eq (u := u) (b₀ := b₀) hu0 hlo hhi
  rw [hP, ell_mul_loOf hu0]
  module

/-- **`hosted_eq_quatSpan` (#2) — Foundations form.**  For an imaginary sedenion
    `s` in the Prop-15 vacuum parametrisation `cdLo s = α·u`,
    `cdHi s = b₀·1 + γ·u` with `u` imaginary and `α² + γ² ≠ 0` (the **non-pole**
    hypothesis), the algebra generated by `{1, s, ℓ}` is EXACTLY the quaternion
    span `ℍ_u = span{1, ℓ, U, ℓU}`, `U = loOf u`.  Both inclusions, so this is
    the equality that `universe_hosts_quaternion` only bounded from above. -/
theorem genByPair_eq_quatSpan_of_param {s : CDAlg ℝ 4} (hs : s.coord 0 = 0)
    {α γ b₀ : ℝ} (hu0 : u.coord 0 = 0)
    (hlo : cdLo s = α • u) (hhi : cdHi s = b₀ • (1 : CDAlg ℝ 3) + γ • u)
    (hk : α ^ 2 + γ ^ 2 ≠ 0) :
    {x : CDAlg ℝ 4 | GenByPair s ell x} = {x : CDAlg ℝ 4 | InQuatSpan (loOf u) x} := by
  have hP := crystal_perp_eq (u := u) (b₀ := b₀) hu0 hlo hhi
  -- `P = s − s₈ℓ` and `P·ℓ` are both hosted …
  have hPmem : GenByPair s ell (s - (s.coord (hiIdx 0)) • ell) := by
    have hrw : s - (s.coord (hiIdx 0)) • ell = s + (-(s.coord (hiIdx 0))) • ell := by
      rw [neg_smul]; abel
    rw [hrw]
    exact GenByPair.add GenByPair.left (GenByPair.smul _ GenByPair.right)
  have hPemem : GenByPair s ell ((s - (s.coord (hiIdx 0)) • ell) * ell) :=
    GenByPair.mul hPmem GenByPair.right
  have hPe : (s - (s.coord (hiIdx 0)) • ell) * ell = (-γ) • loOf u + α • hiOf u := by
    rw [hP]
    simp only [mul_add_left, mul_smul_left]
    rw [loOf_mul_ell, hiOf_mul_ell, loOf_neg]
    module
  -- … hence so are `U` and `Uℓ`, because the matrix `[[α, −γ], [γ, α]]` is invertible.
  have hX : GenByPair s ell (loOf u) := by
    have hc : loOf u
        = (α ^ 2 + γ ^ 2)⁻¹ • (α • (s - (s.coord (hiIdx 0)) • ell)
            + (-γ) • ((s - (s.coord (hiIdx 0)) • ell) * ell)) := by
      have h : α • (s - (s.coord (hiIdx 0)) • ell)
          + (-γ) • ((s - (s.coord (hiIdx 0)) • ell) * ell)
          = (α ^ 2 + γ ^ 2) • loOf u := by
        rw [hPe, hP]; module
      rw [h, smul_smul, inv_mul_cancel₀ hk, one_smul]
    rw [hc]
    exact GenByPair.smul _ (GenByPair.add (GenByPair.smul _ hPmem)
      (GenByPair.smul _ hPemem))
  have hY : GenByPair s ell (hiOf u) := by
    have hc : hiOf u
        = (α ^ 2 + γ ^ 2)⁻¹ • (γ • (s - (s.coord (hiIdx 0)) • ell)
            + α • ((s - (s.coord (hiIdx 0)) • ell) * ell)) := by
      have h : γ • (s - (s.coord (hiIdx 0)) • ell)
          + α • ((s - (s.coord (hiIdx 0)) • ell) * ell)
          = (α ^ 2 + γ ^ 2) • hiOf u := by
        rw [hPe, hP]; module
      rw [h, smul_smul, inv_mul_cancel₀ hk, one_smul]
    rw [hc]
    exact GenByPair.smul _ (GenByPair.add (GenByPair.smul _ hPmem)
      (GenByPair.smul _ hPemem))
  have hZ : GenByPair s ell (ell * loOf u) := by
    rw [ell_mul_loOf hu0, show -(hiOf u) = (-1 : ℝ) • hiOf u by module]
    exact GenByPair.smul _ hY
  ext x
  simp only [Set.mem_setOf_eq]
  constructor
  · exact fun hx => genByPair_subset_quatSpan_of_param (u := u) hs (b₀ := b₀) hu0 hlo hhi x hx
  · rintro ⟨a, b, c, d, rfl⟩
    exact GenByPair.add (GenByPair.add (GenByPair.add
      (GenByPair.smul a GenByPair.one) (GenByPair.smul b GenByPair.right))
      (GenByPair.smul c hX)) (GenByPair.smul d hZ)

/-- `ℍ_{−u} = ℍ_u`: the hosted quaternion span only sees the direction up to
    sign.  (So `u₁ ≠ ±u₂` is exactly the condition for the two spans to differ.) -/
theorem inQuatSpan_neg_dir {x : CDAlg ℝ 4} :
    InQuatSpan (loOf (-u)) x ↔ InQuatSpan (loOf u) x := by
  rw [loOf_neg]
  constructor
  · rintro ⟨a, b, c, d, h⟩
    exact ⟨a, b, -c, -d, by rw [h, cd_mul_neg]; module⟩
  · rintro ⟨a, b, c, d, h⟩
    exact ⟨a, b, -c, -d, by rw [h, cd_mul_neg]; module⟩

/-- **Two `±`-distinct unit imaginary directions span planes meeting in `ℝ·1`.**
    If `a₁·1 + c₁·u₁ = a₂·1 + c₂·u₂` with `u₁, u₂` unit imaginary and
    `u₁ ≠ ±u₂`, then `c₁ = c₂ = 0`. -/
theorem span_one_u_inter {u₁ u₂ : CDAlg ℝ 3} (h1 : u₁.coord 0 = 0) (h2 : u₂.coord 0 = 0)
    (hN1 : N u₁ = 1) (hN2 : N u₂ = 1) (hne : u₁ ≠ u₂) (hne' : u₁ ≠ -u₂)
    {a₁ c₁ a₂ c₂ : ℝ}
    (h : a₁ • (1 : CDAlg ℝ 3) + c₁ • u₁ = a₂ • (1 : CDAlg ℝ 3) + c₂ • u₂) :
    c₁ = 0 ∧ c₂ = 0 := by
  have ha : a₁ = a₂ := by
    have hh := congrArg (fun z : CDAlg ℝ 3 => z.coord 0) h
    simp only [add_coord, smul_coord, one_coord, h1, h2] at hh
    simpa using hh
  have hc : c₁ • u₁ = c₂ • u₂ := by
    rw [ha] at h; exact add_left_cancel h
  by_cases hc1 : c₁ = 0
  · refine ⟨hc1, ?_⟩
    rw [hc1, zero_smul] at hc
    have hN := congrArg N hc
    rw [N_zero, N_smul, hN2, mul_one] at hN
    exact (pow_eq_zero_iff (by norm_num : (2 : ℕ) ≠ 0)).mp hN.symm
  · exfalso
    have hu1 : u₁ = (c₁⁻¹ * c₂) • u₂ := by
      have h' := congrArg (fun z : CDAlg ℝ 3 => (c₁⁻¹ : ℝ) • z) hc
      simp only [smul_smul] at h'
      rw [inv_mul_cancel₀ hc1, one_smul] at h'
      exact h'
    have htsq : (c₁⁻¹ * c₂) ^ 2 = 1 := by
      have hN := congrArg N hu1
      rw [hN1, N_smul, hN2, mul_one] at hN
      exact hN.symm
    have hfac : ((c₁⁻¹ * c₂) - 1) * ((c₁⁻¹ * c₂) + 1) = 0 := by linear_combination htsq
    rcases mul_eq_zero.mp hfac with h' | h'
    · exact hne (by rw [hu1, show c₁⁻¹ * c₂ = 1 by linarith, one_smul])
    · exact hne' (by rw [hu1, show c₁⁻¹ * c₂ = -1 by linarith]; module)

/-- **`universe_intersection_generic_eq_complex` (#3) — Foundations form.**  For
    unit imaginary directions `u₁ ≠ ±u₂`, the two hosted quaternion algebras meet
    in exactly the complex line `span_ℝ{1, ℓ}`:

      `ℍ_{u₁} ∩ ℍ_{u₂} = {a·1 + b·ℓ} ≅ ℂ`.

    Note `ℓ` is in *both*, so the intersection is 2-dimensional, not `ℝ`. -/
theorem quatSpan_inter_eq_complex {u₁ u₂ : CDAlg ℝ 3} (h1 : u₁.coord 0 = 0)
    (h2 : u₂.coord 0 = 0) (hN1 : N u₁ = 1) (hN2 : N u₂ = 1)
    (hne : u₁ ≠ u₂) (hne' : u₁ ≠ -u₂) :
    {x : CDAlg ℝ 4 | InQuatSpan (loOf u₁) x} ∩ {x : CDAlg ℝ 4 | InQuatSpan (loOf u₂) x}
      = {x : CDAlg ℝ 4 | ∃ a b : ℝ, x = a • (1 : CDAlg ℝ 4) + b • ell} := by
  ext x
  simp only [Set.mem_inter_iff, Set.mem_setOf_eq]
  constructor
  · rintro ⟨⟨a₁, b₁, c₁, d₁, hx1⟩, ⟨a₂, b₂, c₂, d₂, hx2⟩⟩
    have hlo1 : cdLo x = a₁ • (1 : CDAlg ℝ 3) + c₁ • u₁ := by
      rw [hx1, cdLo_quatComb a₁ b₁ c₁ d₁ h1]
    have hlo2 : cdLo x = a₂ • (1 : CDAlg ℝ 3) + c₂ • u₂ := by
      rw [hx2, cdLo_quatComb a₂ b₂ c₂ d₂ h2]
    have hhi1 : cdHi x = b₁ • (1 : CDAlg ℝ 3) + (-d₁) • u₁ := by
      rw [hx1, cdHi_quatComb a₁ b₁ c₁ d₁ h1]
    have hhi2 : cdHi x = b₂ • (1 : CDAlg ℝ 3) + (-d₂) • u₂ := by
      rw [hx2, cdHi_quatComb a₂ b₂ c₂ d₂ h2]
    obtain ⟨hc1, -⟩ := span_one_u_inter h1 h2 hN1 hN2 hne hne' (hlo1.symm.trans hlo2)
    obtain ⟨hd1, -⟩ := span_one_u_inter h1 h2 hN1 hN2 hne hne' (hhi1.symm.trans hhi2)
    refine ⟨a₁, b₁, ?_⟩
    rw [hx1, hc1, show d₁ = 0 by linarith]
    module
  · rintro ⟨a, b, rfl⟩
    exact ⟨⟨a, b, 0, 0, by module⟩, ⟨a, b, 0, 0, by module⟩⟩
...
#print axioms loOf_one
#print axioms hiOf_one_eq_ell
#print axioms hiOf_mul_ell
#print axioms crystal_perp_eq
#print axioms genByPair_subset_quatSpan_of_param
#print axioms genByPair_eq_quatSpan_of_param
